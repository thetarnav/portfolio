...
const animationStep = () => {
   timeout = setTimeout(() => {
      ...
      silhouette.value.style.transform = `translate(${random(-12, 12)}px, ${random(-12, 12)}px)`
      lastAnimation = animate(silhouette.value, { opacity: 0 }, { duration: 1, delay: 0.1 })
      animationStep()
   }, random(150, 700))
}

onMounted(() => animationStep())
onUnmounted(() => { clearTimeout(timeout); lastAnimation?.stop() })
</script>
...
<template>
   <section class="mt-16">
      ...
      <div w="58" m="t-8 x-auto">
         ...
         <Button :icon-right="CarbonChat" class="mt-4 bg-red-600">Contact Me</Button>
         ...
         <div class="relative h-64 flex">
            ...
            >
               <glyphs:silhouetteFlat
                  class="test-this absolute inset-0 w-full h-full text-red-800 opacity-90"
               />
               <glyphs:silhouettePen class="absolute inset-0 w-full h-full text-dark-800" />
            </div>
            <h1 class="relative z-10 m-auto text-right text-light-100 normal-case">
               More
               <br />About
               <br />Me
               <br />
               ...
               >
                  <rect x="0.248734" y="50.7515" width="60" height="10" fill="currentColor" />
                  <rect
                     x="60.2487"
                     y="0.751465"
                     width="60"
